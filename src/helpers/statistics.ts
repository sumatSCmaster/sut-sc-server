...
const getNoiceState = (s: string): string => {
  switch(s) {
    ...
    case 'validando': return 'Validando pago';
    case 'ingresardatos': return 'En espera de pago';
    case 'finalizado': return 'Finalizado';
    case 'enrevision': return 'En revisión';
    case 'porrevisar': return 'Por revisar';
...
export const getStats = (user: Usuario) => {
  ...
  else return user.institucion?.id !== 0 ? getOfficialStats(user.institucion?.id) : getMayoraltyStats();
};
...
const getSuperUserStats = async () => {
  ...
  try {
    ...
    const fixedReceivedByDate = new Array(30).fill(null).map((x, i) => receivedByDate.find(e => e.x === moment(Date.now()).subtract(i, 'days').format('DD-MM-YYYY'))
      || ({ x: moment(Date.now()).subtract(i, 'days').format('DD-MM-YYYY'), y: 0 }));
    // GRAFICO 3
    ...
    const fixedLast20Days = new Array(20).fill(null).map((x, i) => last20Days.find(e => e.x === moment(Date.now()).subtract(i, 'days').locale('es').format('DD MMM')) 
      || ({ x: moment(Date.now()).subtract(i, 'days').locale('es').format('DD MMM'), y: 0 }));
    const last12Months = (await client.query(queries.GET_SUPER_PROC_COUNT_LAST_12_MONTHS)).rows.map(r => ({
      x: fixMonth(moment(new Date(r.year, parseInt(r.month) - 1)).locale('es').format('MMM YYYY')),
      y: parseInt(r.count)
    }));
    const fixedLast12Months = new Array(12).fill(null).map((x, i) => last12Months.find(e => e.x === fixMonth(moment(Date.now()).subtract(i, 'months').locale('es').format('MMM YYYY')))
      || ({ x: fixMonth(moment(Date.now()).subtract(i, 'months').locale('es').format('MMM YYYY')), y: 0 }));
    const last5Years = (await client.query(queries.GET_SUPER_PROC_COUNT_LAST_5_YEARS)).rows.map(r => ({
      x: r.year.toString(),
      y: parseInt(r.count)
    }));
    const fixedLast5Years = new Array(5).fill(null).map((x, i) => last5Years.find(e => e.x === moment(Date.now()).subtract(i, 'years').format('YYYY'))
      || ({ x: moment(Date.now()).subtract(i, 'years').format('YYYY'), y: 0 }));
    // RESULT
    return {
      status: 200,
      message: 'Estadisticas obtenidas de manera exitosa',
      stats: formatStats({ totalCount, monthCount, receivedMonthGains, totalToAttend, fixedReceivedByDate, totalCompleted, percentageCompleted,
        completedMonthGains, countByStatus, fixedLast12Months, fixedLast20Days, fixedLast5Years })
    };
  } catch(error) {
    throw {
      status: 500,
