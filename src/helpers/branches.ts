...
export const getTransfersExternalReport = async ({from, to} ,reportName = 'ReporteTransferenciasExterno') => {
  const client = await pool.connect();
  try {
    const result = await client.query(queries.GET_EXTERNAL_TRANSFERS, [from, to]);
