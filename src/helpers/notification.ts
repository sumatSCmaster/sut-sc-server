...
export const sendNotification = async (sender: string, receiver: string, description: string, target: Tarea | null, type: number) => {
  const client = await pool.connect();
  try {
    const result = (await client.query(queries.CREATE_NOTIFICATION, [sender, receiver, description, target ? target.id : 0, type])).rows[0];
