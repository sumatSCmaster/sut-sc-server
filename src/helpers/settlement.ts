...
export const formatBranch = async (branch, contributor, client) => {
  try {
    ...
    return {
      ...
      fechaTimbre: branch.fechaTimbre,
      bancoTimbre: branch.bancoTimbre,
      montoTimbre: branch.montoTimbre,
...
export const insertSettlements = async ({ process, user }) => {
  ...
  try {
    ...
    await client.query(queries.ADD_MOVEMENT, [application.id_solicitud, user.id, `NOW() - interval '4 hours'`, 'creando solicitud', 'IMPUESTO'])
    await client.query('COMMIT');
