...
export const formatBranch = async (branch, contributor, client) => {
  try {
    ...
    servicioMunicipal.exonerado = (await client.query('SELECT EXISTS (SELECT * FROM impuesto.exoneracion_servicios_municipales WHERE id_registro_municipal = $1)', [branch.id_registro_municipal])).rows[0].exists;
