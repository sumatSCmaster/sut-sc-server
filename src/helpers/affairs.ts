...
export const affairInit = async (affair, user) => {
  ...
  try {
    ...
    client.query('COMMIT');
    sendNotification(user.cedula, 'Un trámite ha sido actualizado', 'CREATE_SOCIAL_AFFAIR', caso);
    return {
      status: 201,
...
export const updateAffair = async (affair, user) => {
  ...
  try {
    ...
    sendNotification(user.cedula, 'Un trámite ha sido actualizado', 'UPDATE_SOCIAL_AFFAIR', caso);
