...
export const getRepairYears = async ({ document, reference, docType, user }: { document: string; reference: string; docType: string; user: Usuario }) => {
  ...
  try {
    ...
    const REPApplicationExists = (await client.query(queries.CURRENT_SETTLEMENT_EXISTS_FOR_CODE_AND_RIM_OPTIMIZED, [codigosRamo.REP, branch.id_registro_municipal])).rows[0];
    ...
    let lastREP = (await client.query(queries.GET_LAST_SETTLEMENT_FOR_CODE_AND_RIM_OPTIMIZED, [codigosRamo.RD0, branch.id_registro_municipal])).rows[0];
