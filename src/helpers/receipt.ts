  ...
  try {
    ...
    if (!recibo.rows[0]) {
      return (await client.query(
        `SELECT recibo FROM impuesto.registro_recibo WHERE id_solicitud = $1 AND recibo != ''`,
        [applicationView.id],
      )).rows[0].recibo;
    }
    ...
    return new Promise(async (res, rej) => {
      const pdfDir = resolve(
        __dirname,
        `../../archivos/sedemat/recibo/${applicationView.id}/cierre.pdf`,
      );
      const dir =
        `${process.env.SERVER_URL}/sedemat/recibo/${applicationView.id}/recibo.pdf`;
      let total = breakdownData.reduce((prev, next) => prev + +next.monto, 0);
      const linkQr = await qr.toDataURL(
        dev
          ? dir
          : `${process.env.AWS_ACCESS_URL}/sedemat/recibo/${applicationView.id}/recibo.pdf`,
      ...
      const html = renderFile(
        resolve(__dirname, `../views/planillas/sedemat-recibo.pug`),
        {
          ...
          datos: {
            ...
            items: chunk(
              breakdownData.map((row) => {
                return {
                  descripcion: `${
                    row.datos.descripcion
                      ? row.datos.descripcion
                      : `${row.descripcionRamo} - ${row.descripcionSubramo}`
                  } (${row.datos.fecha.month} ${row.datos.fecha.year})`,
...
export const generateReceiptAgreement = async (
  ...
  try {
    ...
    if (!recibo.rows[0]) {
      return (await client.query(
        `SELECT recibo FROM impuesto.registro_recibo_convenio WHERE id_fraccion = $1 AND recibo != ''`,
        [applicationView.id_fraccion],
      )).rows[0].recibo;
    }
    ...
    return new Promise(async (res, rej) => {
      const pdfDir = resolve(
        __dirname,
        `../../archivos/sedemat/recibo/agreement/${applicationView.id_fraccion}/cierre.pdf`,
      );
      const dir =
        `${process.env.SERVER_URL}/sedemat/recibo/agreement/${applicationView.id_fraccion}/recibo.pdf`;
      ...
      const html = renderFile(
        resolve(__dirname, `../views/planillas/sedemat-RC.pug`),
        {
          moment: require("moment"),
          institucion: "SEDEMAT",
          QR: linkQr,
              ...
              [
                {
                  ...
                  porcion:
                    `${applicationView.porcion}/${applicationView.cantidad}`,
                  descripcion:
                    `${applicationView.descripcionRamo} - ${applicationView.descripcionSubramo} (${
                      date.format("MMMM")
                    } ${date.format("YYYY")})`,
                },
              ],
            ...
            if (err) {
              ...
              try {
                ...
                const bucketParams = {
                  Bucket: process.env.BUCKET_NAME as string,
                  Key:
                    `/sedemat/recibo/agreement/${applicationView.id_fraccion}/recibo.pdf`,
                };
                await S3Client.putObject({
                  ...bucketParams,
                  Body: buffer,
...
export const generateRepairReceipt = async (
  payload: { application: number; breakdownData; total; cashier },
) => {
  ...
  if (!recibo.rows[0]) {
    return (await client.query(
      "SELECT recibo FROM impuesto.registro_recibo WHERE id_solicitud = $1",
      [applicationView.id],
    )).rows[0].recibo;
  }
  const idRecibo = recibo.rows[0].id_registro_recibo;
  ...
  const aforos: any[] = [];
  payload.breakdownData.map((el) =>
    el.desglose.map((x) => aforos.push({ ...x, fecha: el.fecha }))
  );
  try {
    return new Promise(async (res, rej) => {
      const pdfDir = resolve(
      ...
      const dir =
        `${process.env.SERVER_URL}/sedemat/recibo/${applicationView.id}/reparo.pdf`;
      // lest total = breakdownData.reduce((prev,next) => prev + (+next.monto), 0);
      // mainLogger.info('total',total)
      const linkQr = await qr.toDataURL(
        dev
          ? dir
          : `${process.env.AWS_ACCESS_URL}/sedemat/recibo/${applicationView.id}/reparo.pdf`,
        { errorCorrectionLevel: "H" },
      );
      const html = renderFile(
        resolve(__dirname, `../views/planillas/sedemat-reparo.pug`),
        {
          moment: require("moment"),
          institucion: "SEDEMAT",
          QR: linkQr,
          ...
          pdf.create(html, {
            ...
            if (err) {
              ...
              try {
                ...
                await regClient.query(queries.UPDATE_RECEIPT_RECORD, [
                  idRecibo,
                  `${process.env.AWS_ACCESS_URL}/${bucketParams.Key}`,
                ]);
...
export const createOnDemandCertificate = async (
  type: string,
  data: any[],
...
export const createCertificateBuffers = async (
  certInfoArray: any[],
  pugFileName: string,
  bucketKey: string,
): Promise<Buffer[]> => {
  const linkQr = await qr.toDataURL(
    `${process.env.AWS_ACCESS_URL}${bucketKey}`,
    { errorCorrectionLevel: "H" },
  );
