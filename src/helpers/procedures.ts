...
const getNextEventForProcedure = async (procedure, client): Promise<any> => {
  ...
  if (typeof nextEvent === 'string' || procedure.sufijo === 'lae' || (procedure.sufijo === 'sup' && (response.state === 'enproceso' || response.state === 'enrevision')) || (response.state === 'enrevision' && procedure.sufijo === 'lae'))
    return nextEvent;
  if (
