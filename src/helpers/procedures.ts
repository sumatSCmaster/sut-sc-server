  ...
  try {
    ...
    if (procedure.sufijo === 'ompu') {
      ...
      if (aprobado && nextEvent !== 'revisardirector_cr' && nextEvent[aprobado].startsWith('aprobar')) {
        ...
        respState = await client.query(queries.COMPLETE_STATE, [procedure.idTramite, nextEvent === 'aprobar_cr' ? nextEvent : nextEvent[aprobado], datos || null, dir || null, aprobado]);
      } else {
...
export const getNextEventForProcedure = async (procedure, client): Promise<any> => {
  ...
  if (
    ...
    (response.state === 'enrevision_gerente' && procedure.sufijo === 'cr') ||
    (response.state === 'encorreccion' && procedure.sufijo === 'cr')
  )
...
export const procedureEvents = switchcase({
  ...
  ompu: {
    ...
    enrevision: { true: 'finalizar_ompu', false: 'rechazar_ompu' },
    // ingresardatos: 'validar_ompu',
    validando: 'enproceso_ompu',
  },
  ...
  lae: { iniciado: 'validar_lae', validando: 'enproceso_lae', ingresardatos: 'validar_lae', enproceso: { true: 'revisar_lae', false: 'rechazar_lae' }, enrevision: { true: 'revisarrenta_lae', false: 'rechazar_lae' }, enrevision_renta: {true: 'revisardirector_lae', false: 'rechazar_lae'}, enrevision_gerente: {true: 'aprobar_lae', false: 'rechazar_lae'} },
  lic: {
