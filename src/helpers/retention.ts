        ...
        await Promise.all(
          el.items.map(async (x) => {
            try {
              ...
              throw { status: 403, message: `Verifique el rif: ${x.rif} dentro de su declaracion de ${el.fechaCancelada.month} ${el.fechaCancelada.year}, pues este posee un RIM asociado.`, rif: x.rif };
            }
          })
        );
