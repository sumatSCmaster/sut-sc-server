...
export const getEstateByCod = async ({ codCat }) => {
  ...
  try {
    ...
    const cod = inmueble.codigoCatastral.split('');
    console.log(inmueble.codigoCatastral.slice(cod.findIndex(elem => /[a-zA-Z]/.test(elem)), 3), 'TEST MASTER ESTATES')
    inmueble.ambito = (await client.query('SELECT * FROM inmueble.ambito WHERE cod_ambito = $1', [inmueble.codigoCatastral.slice(inmueble.codigoCatastral.split('').findIndex(elem => /[a-zA-Z]/.test(elem)), 2)])).rows[0];
