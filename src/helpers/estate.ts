    ...
    const estatesWithAppraisals = await Promise.all(
      estates.map((row) => {
        return new Promise(async (res, rej) => {
          ...
          res({
            ...row,
            avaluos: (await client.query('SELECT avaluo_terreno AS "avaluoTerreno", avaluo_construccion AS "avaluoConstruccion" FROM impuesto.inmueble_avaluo WHERE id_inmueble = $1 ORDER BY anio DESC LIMIT 1', [row.id])).rows[0],
...
export const getEstateByCod = async ({ codCat }) => {
  ...
  try {
    ...
    inmueble.avaluos = (await client.query('SELECT avaluo_terreno AS "avaluoTerreno", avaluo_construccion AS "avaluoConstruccion" FROM impuesto.inmueble_avaluo WHERE id_inmueble = $1 ORDER BY anio DESC LIMIT 1', [inmueble.id])).rows[0];
