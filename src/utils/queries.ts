...
WHERE ttr.id_tipo_tramite=$1 AND ttr.fisico = false ORDER BY rec.id_recaudo',
  ...
  SELECT ts.*, institucion.nombre_completo AS nombrelargo, institucion.nombre_corto AS 
     ...
     FROM (SELECT * FROM tramite_cte) t
      ...
      tipo_tramite.id_tipo_tramite INNER JOIN institucion ON institucion.id_institucion = tipo_tramite.id_institucion 
      WHERE tipo_tramite.id_institucion = $1 AND state != 'denegado' 
      AND (ts.state NOT IN ('denegado', 'finalizado') OR (ts.state IN ('finalizado') AND EXTRACT(YEAR FROM fechacreacion::date) = EXTRACT(YEAR FROM CURRENT_DATE) ) )
...
`,
  ...
  c.tipo_documento AS "tipoDocumento", c.razon_social AS "razonSocial", f.monto AS "montoFraccion", 
...
`,
  ...
  GET_AGREEMENT_BY_AGREEMENT_ID: 'SELECT * FROM impuesto.convenio WHERE id_convenio = $1',
