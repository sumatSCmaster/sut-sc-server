...
WHERE ttr.id_tipo_tramite=$1 AND ttr.fisico = false ORDER BY rec.id_recaudo',
  ...
  WITH tramite_cte as (
    SELECT * FROM tramite WHERE  fecha_creacion > (NOW() - interval '3 months') AND id_tipo_tramite IN (SELECT id_tipo_tramite FROM tipo_tramite WHERE id_institucion = $1) AND id_tipo_tramite NOT IN (27, 29, 30, 31, 32, 33, 34, 35, 39, 40) ORDER BY fecha_creacion DESC FETCH FIRST 8000 ROWS ONLY
  )
