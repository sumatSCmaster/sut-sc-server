...
WHERE ttr.id_tipo_tramite=$1 AND ttr.fisico = false ORDER BY rec.id_recaudo',
  ...
  GET_SETTLEMENTS_BY_APPLICATION_INSTANCE:
    'SELECT l.*, r.descripcion AS "tipoProcedimiento" FROM impuesto.liquidacion l LEFT JOIN impuesto.subramo sr ON l.id_subramo = sr.id_subramo LEFT JOIN impuesto.ramo r ON sr.id_ramo = r.id_ramo WHERE id_solicitud = $1 ORDER BY l.fecha_vencimiento DESC',
...
l.id_subramo = sr.id_subramo INNER JOIN impuesto.ramo rm ON sr.id_ramo = rm.id_ramo WHERE rm.codigo = $1 AND l.id_registro_municipal =\
 (SELECT id_registro_municipal FROM impuesto.registro_municipal WHERE referencia_municipal = $2 LIMIT 1) ORDER BY fecha_liquidacion DESC',
  ...
  GET_NATURAL_CONTRIBUTOR:
    'SELECT c.id_contribuyente as id, razon_social AS "razonSocial", c.tipo_documento AS "tipoDocumento", c.documento FROM impuesto.contribuyente c WHERE c.tipo_documento = $1 AND c.documento = $2 AND c.tipo_contribuyente = \'NATURAL\';',
  ...
  GET_ESTATE_BY_CODCAT_NAT: `SELECT id_inmueble as id, cod_catastral AS "codigoCatastral", direccion, metros_construccion AS "metrosConstruccion", 
   ...
   FROM inmueble_urbano iu LEFT JOIN impuesto.inmueble_contribuyente_natural icn USING (id_inmueble) 
  ...
  TOTAL_BS_BY_BRANCH_IN_MONTH: `WITH solicitud_view AS (
      ...
      INNER JOIN impuesto.subramo USING (id_subramo)
  ...
  //  Sin fecha proporcionada
  TOTAL_SETTLEMENTS_IN_MONTH: `SELECT l.fecha, l.liq AS liquidado, p.pag AS pagado FROM (
    ...
    WHERE EXTRACT('month' from l.fecha) = EXTRACT('month' from (NOW() - interval '4 hours'))
    AND EXTRACT('year' from l.fecha) = EXTRACT('year' from (NOW() - interval '4 hours')) ORDER BY l.fecha;`,
  //  Con fecha proporcionada
  TOTAL_SETTLEMENTS_IN_MONTH_WITH_DATE: `SELECT l.fecha, l.liq AS liquidado, p.pag AS pagado FROM (
    SELECT fecha_liquidacion AS fecha, COUNT(*) as liq
    FROM impuesto.liquidacion l
    LEFT JOIN impuesto.solicitud s USING (id_solicitud)
    WHERE s.aprobado IS NOT NULL AND s.aprobado = false
