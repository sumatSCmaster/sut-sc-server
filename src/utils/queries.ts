...
const queries = {
  // USUARIO
  GET_USER_ID_BY_RIF: 'SELECT id_usuario FROM usuario WHERE cedula = $1;',
...
`,
  ...
  GET_SOLVENCY_A_RIM_CANDIDATES_BY_RIF: `SELECT EXISTS(SELECT * FROM tramite WHERE datos#>>'{usuario, tipo_documento}' = $1 AND (datos#>>'{usuario, documento}')::VARCHAR = $2 AND datos#>>'{usuario, rim}' IS NULL AND fecha_culminacion > $3 AND id_tipo_tramite = 113)`,
  GET_SOLVENCY_B_RIF_CANDIDATES_BY_RIF: `WITH b AS (SELECT DISTINCT(impuesto.contribuyente.*) FROM usuario LEFT JOIN impuesto.solicitud USING(id_usuario) JOIN impuesto.contribuyente ON usuario.id_contribuyente = impuesto.contribuyente.id_contribuyente WHERE impuesto.solicitud.id_contribuyente NOT IN (SELECT id_contribuyente FROM (SELECT DISTINCT(impuesto.solicitud.*) FROM impuesto.solicitud JOIN impuesto.liquidacion USING (id_solicitud) WHERE id_registro_municipal IS NULL) s WHERE aprobado <> true AND id_contribuyente IS NOT NULL) OR id_solicitud IS NULL)
...
ORDER BY razon_social;`,
  ...
  GET_ALL_TRANSFERS_DIFF_NOW_TOTAL: `SELECT p.id_banco_destino, b.nombre AS nombre_banco, p.metodo_pago, TO_CHAR(p.fecha_de_pago,'YYYY/MM/DD') AS fecha, SUM(p.monto) AS total 
    FROM pago p
  ...
  TOTAL_TRANSFERS_DIFF_NOW:`SELECT p.metodo_pago, 
...
`,
  ...
  GET_VEHICLES_BY_CONTRIBUTOR: `SELECT v.id_vehiculo AS id, mv.nombre AS marca, sv.id_subcategoria_vehiculo AS subcategoria, v.modelo_vehiculo AS modelo,
