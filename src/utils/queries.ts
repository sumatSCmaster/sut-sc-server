const queries = {
  ...
  END_WIZARD: 'UPDATE config SET inicializado = TRUE WHERE inicializado = FALSE RETURNING *',
  // ARBOL
  GET_TREE: 'SELECT arbol_jerarquico FROM config;',
  CLEAR_INDEXES: 'UPDATE cargo SET index_izq = -1, index_der = -1 WHERE nombre != \'Administrador\';',
  INSERT_POSITION: 'INSERT INTO cargo (index_izq, index_der, nombre) \
  VALUES ($1, $2, $3) ON CONFLICT (nombre) \
  DO UPDATE SET index_izq = $1, index_der = $2;',
  SET_TREE: 'UPDATE config SET arbol_jerarquico = $1;',
  GET_CHILDREN: 'SELECT id, nombre FROM cargo WHERE index_izq > $1 AND index_der < $2',
  GET_CHILDREN_USERS: 'SELECT u.* FROM datos_usuario u INNER JOIN usuario x ON u.cedula = x.cedula INNER JOIN cargo c ON c.id = x.id_cargo \
    WHERE c.index_izq > $1 AND c.index_der < $2 AND x.telefono IS NOT NULL;',
  IS_UNDER_IN_TREE: 'WITH cargo_emisor AS (SELECT c.* FROM cargo c INNER JOIN usuario u ON u.id_cargo = c.id WHERE u.cedula = $1), \
  ...
  CREATE_INSTITUTION: 'INSERT INTO institucion(descripcion) VALUES($1) RETURNING *;',
  EDIT_INSTITUTION: 'UPDATE institucion SET descripcion = $1 WHERE id = $2 RETURNING *;',
  // OFICINAS
  GET_INSTITUTION_OFFICES: 'SELECT o.* FROM oficina o INNER JOIN institucion i ON i.id = o.id_institucion WHERE i.id = $1;',
  CREATE_OFFICE: 'INSERT INTO oficina(descripcion, id_institucion) VALUES($1, $2) RETURNING *;',
  EDIT_OFFICE: 'UPDATE oficina SET descripcion = $1, id_institucion = $4 WHERE id = $2 AND id_institucion = $3 RETURNING *;',
  DELETE_OFFICE: 'DELETE FROM oficina WHERE id = $1 AND id_institucion = $2 RETURNING *;',
  // ROLES
  GET_ALL_ROLES: 'SELECT * FROM rol;',
  GET_PERMISSIONS: 'SELECT f.* FROM funcion f INNER JOIN rol_funcion rf ON rf.id_funcion = f.id WHERE rf.id_rol = $1;',
  DELETE_ROLE: 'DELETE FROM rol WHERE id = $1 AND id != (SELECT id FROM rol WHERE nombre = \'Administrador\') RETURNING *;',
  ...
  // INVITACIONES
  GET_ALL_INVITATIONS: 'SELECT i.*, u.nombre_completo FROM invitacion i INNER JOIN usuario u ON u.correo_electronico = i.correo',
  CREATE_INVITATION: 'INSERT INTO invitacion(correo, token) VALUES($1, $2) ON CONFLICT (correo) \
    DO NOTHING RETURNING *;',
  DELETE_INVITATION: 'WITH inv AS (DELETE FROM invitacion WHERE id = $1 RETURNING *) \
    DELETE FROM usuario WHERE correo_electronico = (SELECT correo FROM inv) AND telefono IS NULL RETURNING *',
  ACCEPT_INVITATION: 'WITH invite AS (DELETE FROM invitacion WHERE token = $2 AND id = $1 RETURNING *), \
  ...
  GET_INVITATION_DATA: 'SELECT cedula, nombre_completo as nombre FROM usuario WHERE cedula = $1',
  ...
  CREATE_TASK: 'INSERT INTO tarea(emisor, responsable, status, fecha_asignacion, fecha_entrega, descripcion, titulo) VALUES($1, $2, 1, NOW(), $3, $4, $5) RETURNING *',
  ...
  UPDATE_TASK_STATUS: 'UPDATE tarea SET status = $1 WHERE id = $2 RETURNING id;',
  UPDATE_TASK_INFO: 'WITH updated as (UPDATE tarea SET titulo = $1, descripcion = $2 WHERE id = $3) \
    SELECT * FROM datos_tarea WHERE id = $3;',
  GET_RATE_PENDING: 'SELECT * FROM datos_tarea WHERE status = 4 AND emisor = $1;',
  RATE_TASK: 'SELECT * FROM calificar_tarea($1, $2);',
  // COMENTARIOS
  CAN_COMMENT: 'SELECT true AS can_comment FROM tarea WHERE (responsable = $1 OR emisor = $1) AND id = $2;',
  GET_TASK_COMMENTS: 'SELECT * FROM comentario WHERE target = $1 AND tipo = 1',
  CREATE_COMMENT: 'INSERT INTO comentario(emisor, target, tipo, descripcion, url_archivo) VALUES($1, $2, 1, $3, $4) RETURNING *;',
  EDIT_COMMENT: 'UPDATE comentario SET descripcion = $1 WHERE id = $2 RETURNING *;',
  CAN_EDIT_COMMENT: 'SELECT id FROM comentario WHERE target = $1 AND tipo = 1 AND emisor = $2',
  DELETE_COMMENT: 'DELETE FROM comentario WHERE id = $1 RETURNING *;',
  // PERMISOS
  GET_ALL_PERMISSIONS: 'SELECT * FROM funcion;',
  HAS_PERMISSION: 'SELECT * FROM rol_funcion WHERE id_rol = (SELECT id_rol FROM usuario WHERE cedula = $1) AND id_funcion = $2',
  GET_USET_PERMISSIONS: 'SELECT id_funcion FROM rol_funcion WHERE id_rol = (SELECT id_rol FROM usuario WHERE cedula = $1);',
  // NOTIFICACIONES
  GET_USER_NOTIFICATIONS: 'SELECT * FROM datos_notificacion WHERE receptor = $1 ORDER BY fecha DESC LIMIT 100;',
  CREATE_NOTIFICATION: 'INSERT INTO notificacion(emisor, receptor, descripcion, target, tipo) VALUES($1, $2, $3, $4, $5) RETURNING *;',
  GET_NOTIFICATION_BY_ID: 'SELECT * FROM datos_notificacion WHERE id = $1;',
  HAS_UNREAD_NOTIF: 'SELECT id FROM notificacion WHERE status = FALSE AND receptor = $1;',
  MARK_ALL_AS_READ: 'UPDATE notificacion SET status = TRUE WHERE status = FALSE AND receptor = $1;',
  // PROYECTOS
  ...
  CREATE_PROJECT: 'INSERT INTO proyecto(fecha_culminacion, status, id_institucion, responsable, nombre, direccion, id_parroquia, longitud, latitud,\
