...
import { generateCashierReport, generateAllCashiersReport, getCashierReceipts, generateOneCashierReport } from '@helpers/cashier';
...
router.post('/one', authenticate('jwt'), async (req, res) => {
  const { day, tipoDocumento, documento } = req.body;
  const [error, data] = await fulfill(generateOneCashierReport(req.user, { day, tipoDocumento, documento }));
  if (error) res.status(500).json({ error, status: 500 });
  if (data) res.status(200).json({ status: 200, data });
});
