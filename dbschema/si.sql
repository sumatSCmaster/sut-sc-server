    ...
    --Obtiene el id del pago
        ...
        IF idPago IS NOT NULL THEN
            ...
            IF (SELECT concepto FROM pago WHERE id_pago = idPago) = 'IMPUESTO' THEN
                ...
                select row_to_json(row)::jsonb into dataPago from (select pago.id_pago AS id, pago.monto, pago.aprobado, pago.id_banco AS idBanco, pago.id_procedimiento AS idProcedimiento, pago.referencia, pago.fecha_de_pago AS fechaDePago, pago.fecha_de_aprobacion AS fechaDeAprobacion, solicitud.documento, solicitud.nacionalidad, solicitud.rim, solicitud.aprobado, solicitud.monto_total from pago 
